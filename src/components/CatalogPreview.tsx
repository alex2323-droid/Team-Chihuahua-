...
export default function CatalogPreview({
  ...
  const getThemeClasses = () => {
    ...
    switch (settings.theme) {
      case 'premium':
        return 'bg-neutral-950 text-neutral-100 min-h-screen';
      case 'retro':
        return 'bg-[#FAF7F0] dark:bg-neutral-950 text-neutral-800 dark:text-neutral-100 min-h-screen font-serif';
      case 'minimalist':
        return 'bg-white dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 min-h-screen';
      default:
        return 'bg-neutral-50/50 dark:bg-neutral-950 text-neutral-800 dark:text-neutral-100 min-h-screen';
    }
  };

  const getCardClasses = () => {
    if (settings.theme === 'dark') {
      return 'bg-black border border-neutral-800 rounded-xl overflow-hidden shadow-lg text-white';
    }
    switch (settings.theme) {
      case 'premium':
        return 'bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-lg';
      case 'retro':
        return 'bg-white border-2 border-neutral-800 rounded-none overflow-hidden dark:bg-neutral-900 dark:border-neutral-800';
      case 'minimalist':
        return 'bg-white rounded-none overflow-hidden hover:opacity-95 dark:bg-neutral-900';
      default:
        return 'bg-white border border-neutral-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition dark:bg-neutral-900 dark:border-neutral-800 dark:text-white';
    }
  };
  ...
  const content = (
    <div className={`${getThemeClasses()} pb-24`} id="catalog-storefront-wrapper">
      {/* Header Comercial */}
      <div className={`p-6 border-b ${settings.theme === 'premium' || settings.theme === 'dark' ? 'border-neutral-800 bg-black' : 'border-neutral-100 dark:border-neutral-800 bg-white dark:bg-neutral-900'}`}>
          ...
          <div className="flex items-center gap-4">
            {business.logoUrl ? (
              <img
                ...
                alt="Logo negocio"
                className="w-16 h-16 rounded-xl object-cover border border-neutral-100 dark:border-neutral-800 shadow-sm"
              />
            ) : (
          ...
          <div className="flex items-center gap-2 no-print">
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg transition border ${
                settings.theme === 'premium' || settings.theme === 'dark'
                  ? 'border-neutral-700 bg-neutral-800 text-neutral-200 hover:bg-neutral-700'
                  : 'border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-200'
              }`}
            >
            ...
            <input
              ...
              className={`w-full pl-9 pr-4 py-2 text-xs rounded-lg focus:outline-none transition ${
                settings.theme === 'premium' || settings.theme === 'dark'
                  ...
                  : 'bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-850 text-neutral-800 dark:text-neutral-100 focus:ring-1 focus:ring-neutral-400 dark:focus:ring-neutral-700'
              }`}
            />
          </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition whitespace-nowrap ${
                    selectedCategory === cat
                      ...
                      : 'bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 text-neutral-600 dark:text-neutral-300'
                  }`}
            ...
            {filteredProducts.map((p) => {
              ...
              return (
                <div key={p.id} className={getCardClasses()}>
                  {/* Foto del Producto */}
                  <div className="aspect-square relative bg-neutral-100 dark:bg-neutral-950 overflow-hidden group">
                    <img
                      src={p.imageUrl}
                      alt={p.name}
                    ...
                    {!isAgotado && (
                      <button
                        ...
                        className="absolute bottom-2 right-2 p-2 rounded-full text-white shadow-md hover:scale-105 active:scale-95 transition cursor-pointer"
                        style={primaryBg}
                      >
                  ...
                  <div className="p-4 space-y-1.5 flex flex-col justify-between">
                    ...
                    <div className="space-y-2 mt-2 pt-2 border-t border-dashed border-neutral-200 dark:border-neutral-800">
                      ...
                      <div className="flex items-center justify-between gap-2 pt-1">
                        ...
                        {qtyInCart > 0 && (
                          <div className="flex items-center gap-1.5 bg-neutral-100 dark:bg-neutral-850 text-neutral-900 dark:text-neutral-100 py-1 px-2 rounded-lg text-xs font-bold">
                            <button onClick={() => removeFromCart(p.id)} className="hover:text-red-600 cursor-pointer">
                              <Minus className="w-3 h-3" />
                            </button>
                            <span>{qtyInCart}</span>
                            <button onClick={() => addToCart(p.id)} className="hover:text-green-600 cursor-pointer">
        ...
        {(business.paymentMethods.length > 0 || business.additionalInfo) && (
          <div className={`p-6 rounded-xl border mt-10 ${
            settings.theme === 'premium' || settings.theme === 'dark'
              ...
              : 'bg-white dark:bg-neutral-900 border-neutral-100 dark:border-neutral-805 shadow-sm'
          }`}>
                    ...
                    {business.paymentMethods.map((m) => (
                      <span
                        ...
                        className={`px-2.5 py-1 rounded-full font-medium ${
                          settings.theme === 'premium' || settings.theme === 'dark' ? 'bg-neutral-800 text-neutral-300' : 'bg-neutral-50 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-700'
                        }`}
                      >
                        {m}
                      </span>
      ...
      {cartCount > 0 && (
        <div className="fixed bottom-6 inset-x-4 max-w-lg mx-auto bg-neutral-900 dark:bg-neutral-950 text-white rounded-xl shadow-xl border border-neutral-800 p-4 flex items-center justify-between gap-4 z-50 animate-bounce-short">
          <div className="flex items-center gap-3">
            <div className="relative bg-neutral-800 dark:bg-neutral-900 p-2.5 rounded-lg">
      ...
      {isCartOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`w-full max-w-md rounded-xl shadow-2xl flex flex-col max-h-[85vh] ${
            settings.theme === 'premium' || settings.theme === 'dark' ? 'bg-neutral-900 border border-neutral-800 text-white' : 'bg-white text-neutral-800'
          }`}>
            <div className="p-4 border-b border-black/10 flex items-center justify-between">
              ...
              <button onClick={() => setIsCartOpen(false)} className="p-1 hover:bg-black/10 rounded cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
                  ...
                  <div key={p.id} className="flex items-center gap-3 justify-between pb-3 border-b border-neutral-100 dark:border-neutral-800">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <img 
                        ...
                        referrerPolicy="no-referrer" 
                        onError={(e) => handleImageError(e, p.imageUrl)}
                        className="w-10 h-10 rounded object-cover border dark:border-neutral-800" 
                      />
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => removeFromCart(p.id)} className="p-1 hover:bg-black/5 rounded text-red-500 cursor-pointer">
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="text-xs font-bold">{qty}</span>
                      <button onClick={() => addToCart(p.id)} className="p-1 hover:bg-black/5 rounded text-green-600 cursor-pointer">
                        <Plus className="w-3 h-3" />
                      </button>
                    </div>
                  </div>
                      ...
                      <button
                        ...
                        className={`px-2 py-1.5 rounded-lg text-[10px] font-medium border text-left transition ${
                          selectedPaymentMethod === m
                            ...
                            : 'bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-750 text-neutral-600 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-850'
                        }`}
                      >
                        {m}
                      </button>
            ...
            <div className="p-4 border-t border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950 space-y-3 rounded-b-xl">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={clearCart}
                  className="px-4 py-2 border border-neutral-300 dark:border-neutral-800 text-xs font-bold rounded-lg hover:bg-black/5 transition text-center text-neutral-500"
                >
                  Vaciar
                </button>
                <button
  ...
  return (
    <div className="space-y-4" id="catalog-preview-container">
      <div className="bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm text-neutral-800 dark:text-neutral-100 transition-colors">
        <div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => setIsPdfModalOpen(true)}
            className="px-3 py-1.5 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-200 text-xs font-semibold transition flex items-center gap-1.5 shadow-2xs cursor-pointer"
          ...
          <div className="flex items-center gap-1.5 border border-neutral-200 dark:border-neutral-850 rounded-lg p-1 bg-neutral-50 dark:bg-neutral-950">
            <button
              onClick={() => setDeviceMode('mobile')}
              className={`p-1.5 rounded transition cursor-pointer ${
                deviceMode === 'mobile' ? 'bg-white dark:bg-neutral-900 shadow-sm text-neutral-900 dark:text-white' : 'text-neutral-400 hover:text-neutral-600'
              }`}
              title="Vista Móvil"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded transition cursor-pointer ${
                deviceMode === 'desktop' ? 'bg-white dark:bg-neutral-900 shadow-sm text-neutral-900 dark:text-white' : 'text-neutral-400 hover:text-neutral-600'
              }`}
      ...
      {deviceMode === 'mobile' ? (
        <div className="flex justify-center py-4 bg-neutral-50/50 dark:bg-neutral-950 rounded-2xl border border-neutral-100 dark:border-neutral-850 shadow-inner">
          <div className="w-[375px] h-[750px] bg-white dark:bg-black rounded-[40px] border-[12px] border-neutral-900 overflow-hidden shadow-2xl relative flex flex-col">
        ...
        <div className="border border-neutral-200 dark:border-neutral-800 rounded-xl overflow-hidden shadow-md max-h-[750px] overflow-y-auto">
          {content}
        </div>
      )}
